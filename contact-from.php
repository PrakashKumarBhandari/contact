...
if ( ! empty( $post->id ) ) {
	$nonce_action = 'contact23-update-contact_' . $post->id;
} else {
	$nonce_action = 'contact23-add-contact';
}

?>
<div>

<h1><?php echo esc_html( __( 'Edit Contact', 'contact234' ) ); ?></h1>

<?php do_action( 'contact234_admin_updated_message', $post ); ?>
...
<?php else : ?>
<label class="hide-if-no-js" style="visibility:hidden" id="title-prompt-text" for="title"><?php echo esc_html( __( 'Enter email here', 'contact234' ) ); ?></label>
